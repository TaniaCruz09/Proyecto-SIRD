...
// export async function saveDocente(docente: DocentePayload): Promise<{ id: number }> {
//     const endPoint = `/docentes`;
...
export async function updateDocente(id: number, formData: FormData) {
  const endPoint = `/docentes/${id}`;
    console.log(formData, 'este es el docente que se envia al back para actualizar')
